...
class PathTraversalError(SourceRecallError):
    """A caller-supplied file path escaped the repository root.

    Raised by ``IndexBuilder.refresh(files=[...])`` when an entry is
    not repo-contained after symlink resolution. Returned to HTTP
    callers as 400 (no internal path is leaked).

    @param supplied: The offending path string.
    @param resolved: The resolved absolute path (only useful in logs).
    """

    def __init__(self, supplied: str, resolved: str = "") -> None:
        self.supplied = supplied
        self.resolved = resolved
        msg = f"Path escapes repository root: {supplied!r}"
        if resolved:
            msg += f" (resolved to {resolved})"
        super().__init__(msg)
